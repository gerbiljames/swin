# This is an amended version of pygame_framework.py (provided with pybox2d)
# Edited by Becky Naylor Dept of Electronics, University of York for Pi-Swarm simulation
# Changes: Added quit function, changed window size and colour of solid shapes
...
#        self.screen = pygame.display.set_mode((w,h),flags^FULLSCREEN,bits)
        ...
        while running:
            ...
            if self.renderer:
                self.renderer.flags=dict(
                        drawShapes=self.settings.drawShapes,
                        drawJoints=self.settings.drawJoints,
                        drawAABBs =self.settings.drawAABBs,
                        drawPairs =self.settings.drawPairs,
                        drawCOMs  =self.settings.drawCOMs,
            ...
            if self.renderer:
                ...
                if self.mouseJoint:
                    ...
                    self.renderer.DrawPoint(p1, self.settings.pointSize, self.colors['mouse_point'])
                    self.renderer.DrawPoint(p2, self.settings.pointSize, self.colors['mouse_point'])
                    self.renderer.DrawSegment(p1, p2, self.colors['joint_line'])
     
                # Draw the slingshot bomb
                if self.bombSpawning:
                    self.renderer.DrawPoint(self.renderer.to_screen(self.bombSpawnPoint), self.settings.pointSize, self.colors['bomb_center'])
                ...
                if self.settings.drawContactPoints:
                    for point in self.points:
                        if point['state'] == b2_addState:
                            self.renderer.DrawPoint(self.renderer.to_screen(point['position']), self.settings.pointSize, self.colors['contact_add'])
                        elif point['state'] == b2_persistState:
                            self.renderer.DrawPoint(self.renderer.to_screen(point['position']), self.settings.pointSize, self.colors['contact_persist'])
     
                if self.settings.drawContactNormals:
                    for point in self.points:
                ...
                self.Draw() #Call user added draw function now 
    ...
    #Added function to allow to quit in code
    def QuitPygame(self):
        print "quitting"
        quitev = pygame.event.Event(QUIT)
        pygame.event.post(quitev)
